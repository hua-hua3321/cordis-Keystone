...
**台账注记（2026-08-16 清理）**：①原表尾残留 P3-P13 ⏳ 模板行（与上方 ✔ 行重复）已删除；②原表尾孤立行「P3 门控（GateTests）」已删除（凭证由 P3-1..P3-4 正式行取代）；③P6-5 的凭证 `EntryGroupTests` 已随 P65（19 号审计 P2-9）删除 standalone `EntryGroup` 双实现——组级事务语义现由宿主 `ApplyDiffTransactionallyAsync` 唯一承担（W59-01/W64-01），历史验收时点该测试存在，结论不受影响。
...
| 任务 | 目标 | 产出 | 状态 |
|------|------|------|------|
| T1 全源枚举 | Cordis 核心 9 文件（2693 行）+ plugin-include（377）+ plugin-loader（~1000）+ cosmokit/bin.js 逐文件行为点枚举 | 54 项登记（双侧 文件:行号；口径见 19 §0——P0 7/P1 7/D 9/P2 31） | ✔ 2026-08-16 |
