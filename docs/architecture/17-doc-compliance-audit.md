...
| DC-3 | ADR-0005、09 §4 | 卸载五步闸门（拒绝新任务/在途排空/超时审计） | 只做三步（置 UNLOADING→dispose→摘注册），缺拒绝/排空/超时 | ❌ |
...
| DC-6 | 02 §3、ADR-0007 | rebind=报错 + 热重载服务保持 | Register 用 TryAdd 静默吞重复；热重载旧实例误删新注册 | ❌ |
