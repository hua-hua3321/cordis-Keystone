...
> **结论**：P57 核心成果（两档域 schema/发现层投影/disposer 幂等/移动精确回插/防回环）经对抗验证**正确**；但发现 **7 项 P0 正确性缺陷**（多落在近期 CA 实施的未测路径）、7 项 P1 竞态、9 项语义偏差待决策、31 项 P2 对齐/文档项。
> **计数口径（统一）**：合计 **54 项登记行** = P0 七（§1）+ P1 竞态七（§2）+ 语义偏差九（§3）+ P2 三十一（§4）。P2 多行合并多个子发现（一行内并列 SV/CF/EV/LG/LD/IN 源级编号，全文源级编号共 56 个）；引用总数一律以本口径为准（不使用历史草稿的 108/116 等汇总数）。
